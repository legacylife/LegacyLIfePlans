  ...
  checkAvailableSpace = (userData={}, callback) => {
    this.subscription.checkSubscription( userData, ( returnArr )=> {
      ...
      let defaultSpace      = Number(returnArr.defaultSpace),
          ...
          /*totalSpaceAlloted = 10485760, /total alloted space in bytes i.e 10 MB = 10485760 Bytes */
      ...
      if( totalUsedSpace < totalSpaceAlloted ) {
        ...
        if( remainingSpace >= 0 && remainingSpace <= 524288000 ) {
          // for - If remaining space less than equal to 5 MB
        //if( remainingSpace >= 0 && remainingSpace <= 5242880 ) {
